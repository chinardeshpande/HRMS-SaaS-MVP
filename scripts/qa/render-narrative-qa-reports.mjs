...
const attendanceLeaveScenarios = [
  ...
  },
  {
    ...
    actual: 'Post-deploy strict QA verified the backend bulk update path and asserted both the Mass Attendance Update and Sync Attendance modals render. CSV-backed sync now saves valid rows through the backend bulk-update path.',
  ...
  },
  {
    ...
    actual: 'Post-deploy strict QA verified attendance statistics, department attendance statistics, leave statistics, leadership views, and the HR Reports tab without query failures.',
...
const lifecycleScenarios = [
  ...
  },
  {
    ...
    actual: 'Post-deploy strict QA verified `/performance/my-reviews` and asserted the employee page shows employee self-service language such as My Performance and My Reviews without HR-only controls.',
  ...
  },
  {
    ...
    actual: 'Post-deploy strict QA verified `/exit/my-case` returns safely for employees with no active resignation and asserted Submit Resignation is visible in the employee self-service view.',
...
await renderReport({
  ...
  verificationBoundary: 'Important boundary: this is now post-deploy strict evidence. Earlier baseline gaps remain documented separately in the QA evidence audit.',
  executiveNarrative: 'This document follows the daily employee and HR operating rhythm: employees record attendance and apply for leave, managers approve exceptions, HR intervenes where required, and leadership reviews company-level health. The post-deploy strict QA pass verified the core workflows, backend persistence paths, reporting APIs, and role-specific visual surfaces.',
...
await renderReport({
  ...
  verificationBoundary: 'Important boundary: this is now post-deploy strict evidence. Earlier baseline gaps remain documented separately in the QA evidence audit.',
  executiveNarrative: 'This document follows the broader employee lifecycle: hiring and onboarding, probation tracking, performance review cycles, and eventual exit/offboarding. The post-deploy strict QA pass verified HR, manager, employee, and admin workflows, including the previously missing employee Performance and Exit self-service paths.',
