...
| ACV-GAP-003 | Documents | Data Model Gap | Company HR/compliance document vault is not explicit. | Company document vault with categories, ownership, access, expiry metadata, and audit. | P1 |
...
| ACV-GAP-007 | Audit | Configuration Gap | Audit model exists but sensitive operations are not systematically covered. | Audit coverage for employee, document, compensation, settings, role, import, and email operations. | P1 |
