...
export const AnnualReviewForm: React.FC<AnnualReviewFormProps> = ({
  review,
  onSubmit,
  isSubmitting = false,
  isManagerView = false,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          rows={4}
          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="Reflect on your performance throughout the entire year"
          disabled={isSubmitting}
        />
      </div>
      ...
      {isManagerView && (
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="Provide comprehensive feedback on employee's annual performance"
            disabled={isSubmitting}
          />
        </div>
      )}
          ...
          {(formData.achievements || ['']).map((achievement, index) => (
            <div key={index} className="flex gap-2">
              <input
                ...
                placeholder="Describe an annual achievement"
                disabled={isSubmitting}
              />
          ...
          {(formData.challenges || ['']).map((challenge, index) => (
            <div key={index} className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Describe a challenge faced during the year"
                disabled={isSubmitting}
              />
